...
async function fetchData() {
    let users = [];
    try{
        users = await requestGET('/SAE4_RefactoringSiteADIIL/index.php?page=api_users');
    } catch (error) {
...
async function saveUser(id_user){
    ...
    try {
        await requestPUT('/SAE4_RefactoringSiteADIIL/index.php?page=api_users&id=' + id_user.toString(), data);
        ...
        await requestPUT('/SAE4_RefactoringSiteADIIL/index.php?page=api_userole&id=' + id_user.toString(), roles);
...
async function deleteUser(id_user){
    showLoader();
    await requestDELETE(`/SAE4_RefactoringSiteADIIL/index.php?page=api_users&id=${id_user}`);
...
async function selectUser(id_member, li){
    ...
    const user = await requestGET(`/SAE4_RefactoringSiteADIIL/index.php?page=api_users&id=${id_member}`);
    const defaultImagePath = '/SAE4_RefactoringSiteADIIL/public/admin/ressources/default_images/user.jpg';
    ...
    if (user.pp_membre && user.pp_membre.startsWith('http')) {
        ...
        prop_img.src = '/SAE4_RefactoringSiteADIIL/files/' + user.pp_membre;
    } else {
    ...
    document.getElementById('prop_img_edit').onclick = async ()=>{
        ...
        try {
            ...
            const response = await fetch('/SAE4_RefactoringSiteADIIL/index.php?page=api_users&action=update_image&id=' + id_member.toString(), {
                method: 'POST',
                body: formData
            });
    ...
    const roles = await requestGET('/SAE4_RefactoringSiteADIIL/index.php?page=api_roles'); 
    const user_roles = (await requestGET(`/SAE4_RefactoringSiteADIIL/index.php?page=api_userole&id=${id_member}`)).map(role => role.id_role); 
    roles.forEach(role => {
...
new_btn.onclick = async ()=>{
    showLoader();
    try {
        const result = await requestPOST('/SAE4_RefactoringSiteADIIL/index.php?page=api_users');
