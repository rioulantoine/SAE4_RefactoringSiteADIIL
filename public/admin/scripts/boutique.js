...
async function fetchData() {
    let articles = [];
    try{
        articles = await requestGET('/SAE4_RefactoringSiteADIIL/index.php?page=api_item');
    } catch (error) {
...
async function saveArticle(id_article){
    ...
    try {
        await requestPUT('/SAE4_RefactoringSiteADIIL/index.php?page=api_item&id=' + id_article.toString(), data);
...
async function deleteArticle(id_article){
    showLoader();
    await requestDELETE(`/SAE4_RefactoringSiteADIIL/index.php?page=api_item&id=${id_article}`);
...
async function selectArticle(id_article, li){
    ...
    const article = await requestGET(`/SAE4_RefactoringSiteADIIL/index.php?page=api_item&id=${id_article}`);
    const defaultImagePath = '/SAE4_RefactoringSiteADIIL/public/admin/ressources/default_images/boutique.png';
    ...
    if (article.image_article && article.image_article.startsWith('http')) {
        ...
        prop_image.src = '/SAE4_RefactoringSiteADIIL/files/' + article.image_article;
    } else {
    ...
    document.getElementById('prop_image_edit').onclick = async ()=>{
        ...
        try {
            ...
            const response = await fetch('/SAE4_RefactoringSiteADIIL/index.php?page=api_item&action=update_image&id=' + id_article.toString(), {
                method: 'POST',
                body: formData
            });
...
new_btn.onclick = async ()=>{
    showLoader();
    try {
        const id = await requestPOST('/SAE4_RefactoringSiteADIIL/index.php?page=api_item');
